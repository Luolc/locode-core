//! Unicode-confusables detection — verbatim port of Grok Build's
//! `xai-grok-tools/src/util/unicode_confusables.rs` (Task 26,
//! `search_replace` current-default surface; the confusable *diagnostic*
//! is unconditional in grok — only the fallback matching is config-gated
//! and stays unported, default-off).
