...
3. **Publish all 9 crates to crates.io** (the workflow does **not** — it's agent-driven),
   ...
   steps). `locode-tui` and `locode-app` are `publish = false` — 9 published, 11 in the
   workspace (`locode-instructions` added 2026-07-24; `locode-skills` will make it 10/12). Auth is `~/.cargo/credentials.toml`; new-version publishes are not
