...
7. Extension seams exist — some now filled. **Shipped since v0** (as of 2026-07-29): a second `Provider` wire (OpenAI Responses) and streaming (ADR-0021); the **`claude`** (Task 20) and **`codex`** (Task 19) harness packs, the latter carrying `apply_patch`; the interactive app (ADR-0019/0022); the `~/.locode` home with settings + resumable traces (ADR-0024); agent skills (ADR-0025); slash commands (ADR-0026); mid-run user input (ADR-0028); the session picker behind `--resume`/`/resume` (ADR-0029). **Still unimplemented**: our own **`locode` best-of pack** (the last pack — the `opencode` port was cancelled 2026-07-24), background tasks + subagents, parallel tools (ADR-0027, draft), compaction, sandbox, MCP.
